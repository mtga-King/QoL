exports.mod = (mod_info) => {
	...
	if (config.FoundInRaid == true) {
		if (users.trading.buyItemsMarkedFound == false) {
			users.trading.buyItemsMarkedFound = true;
			logger.logInfo(`*Most* Purchased items marked Found In Raid`);
		}
	}
	else if (config.FoundInRaid == false) {
		users.trading.buyItemsMarkedFound = false;
		logger.logInfo(`Purchased Items are Default`);
	}
	...
	if (config.UnlockCosmetics == true) {
		...
		logger.logInfo(`Cosmetics require only Money`);
	}
	...
	if (config.InstantDeploy == true) { //if true
		if (globals.data.config.TimeBeforeDeploy === 20) {
			globals.data.config.TimeBeforeDeploy = 0
		}
		logger.logInfo(`Time Before Deploy (1/2) removed`);
		if (globals.data.config.TimeBeforeDeployLocal === 10) {
			globals.data.config.TimeBeforeDeployLocal = 0
		}
		logger.logInfo(`Time Before Deploy (2/2) removed`);
	}
	if (config.InstantDeploy == false) { //if false, reset to default
		if (globals.data.config.TimeBeforeDeploy > 20) {
			globals.data.config.TimeBeforeDeploy = 20
		}
		logger.logInfo(`Time Before Deploy (1/2) default`);
		if (globals.data.config.TimeBeforeDeployLocal > 10) {
			globals.data.config.TimeBeforeDeployLocal = 10
		}
		logger.logInfo(`Time Before Deploy (2/2) default`);
	}
	...
	if (config.RemoveRestrictionLimitsInRaid == true) {
		...
		logger.logInfo(`InRaid Item restrictions removed`);
		...
		logger.logInfo(`InRaid Item restrictions default`);
	}
	...
	if (config.RunThroughRequirement.EditRunThrough == true) {
		...
		_match_end.survivedMult = 0.75;
		logger.logInfo(`Run-Thru requirements reduced`)
	} else if (config.RunThroughRequirement.EditRunThrough == false) {
		_match_end.survived_exp_requirement = 300;
		_match_end.survived_seconds_requirement = 600;
		_match_end.survivedMult = 1.5;
		logger.logInfo(`Run-thru requirements defaulted`)
