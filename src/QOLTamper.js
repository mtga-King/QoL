exports.mod = (mod_info) => {
	...
	const globals = fileIO.readParsed(global.db.base.globals).data;
	const trading = fileIO.readParsed(global.db.user.configs.gameplay).trading;

	//Set all Items as FoundInRaid (FIR)
	if (config.FoundInRaid && !trading.buyItemsMarkedFound) {
		trading.buyItemsMarkedFound = true;
	} else {
		trading.buyItemsMarkedFound = false;
	}
	...
	//Config.node Loop
	if (config.InstantDeploy) { //if true
		globals.config.TimeBeforeDeploy = 0 //then make it equal 0 (zero)
		globals.config.TimeBeforeDeployLocal = 0 //make 0 (zero)
	}
	else { //if false, reset to default
		globals.config.TimeBeforeDeploy = 20
		globals.config.TimeBeforeDeployLocal = 10

	}

	//Remove Item Limit Restrictions

	if (config.RemoveRestrictionLimitsInRaid) {
		globals.config.RestrictionsInRaid = []; //remove restrictions
	...
	const _match_end = globals.config.exp.match_end; //match_end node var

	if (config.RunThroughRequirement.EditRunThrough) {
